...
then
  ...
  then
    ...
    HOMEBREW_PREFIX="/Users/vitek.dylan/homebrew"
  ...
  INSTALL=("/usr/bin/install" -d -o "vitek.dylan" -g "staff" -m "0755")
else
...
execute_sudo() {
  ...
  then
    ...
    ohai "/usr/bin/bash" "${args[@]}"
    execute "/usr/bin/bash" "${args[@]}"
  else
    ohai "${args[@]}"
    execute "${args[@]}"
  fi
